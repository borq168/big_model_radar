/**
 * Radar Forge: daily digest for AI CLI tools and OpenClaw.
 *
 * Env vars:
...
const INTEGRATED_DAILY_SOURCE_MAX_CHARS = 3500;

interface SavedReport extends DailySourceReport {
  content: string;
}

function compactForIntegratedDaily(content: string, lang: "zh" | "en"): string {
  const footerPattern =
    lang === "en"
      ? /\n\n---\n\*This digest is auto-generated by \[(?:Radar Forge|Big Model Radar|agents-radar)\][\s\S]*$/m
      : /\n\n---\n\*本日报由 \[(?:Radar Forge|Big Model Radar|agents-radar)\][\s\S]*$/m;

  const compacted = stripThinkBlocks(content)
    .replace(/<details\b[\s\S]*?<\/details>/gi, "\n\n")
    .replace(footerPattern, "")
    .replace(/\n{3,}/g, "\n\n")
    .trim();

  if (compacted.length <= INTEGRATED_DAILY_SOURCE_MAX_CHARS) return compacted;
  const note = lang === "en" ? "\n...[source truncated for integrated daily]" : "\n...[来源报告已截断]";
  return compacted.slice(0, INTEGRATED_DAILY_SOURCE_MAX_CHARS) + note;
}

function makeSavedReport(
  id: string,
  title: string,
  filename: string,
  content: string,
  ...
  return {
    ...
    title,
    filename,
    content: compactForIntegratedDaily(content, lang),
  };
}
...
async function generateSummaries(
  ...
  const trendingNoData =
    lang === "en"
      ...
      : "⚠️ 今日热榜数据获取失败，无法生成报告。";
  const trendingFailed = lang === "en" ? "⚠️ Trending report generation failed." : "⚠️ 热榜报告生成失败。";
...
async function saveContentReport(
  ...
  if (hasNewContent) {
    ...
    try {
      ...
      return makeSavedReport(reportBaseId, reportTitle, fileName, webContent, lang);
    } catch (err) {
...
async function saveTrendingReport(
  ...
  if (digestRepo) {
    const trendingTitle =
      lang === "en" ? `📈 GitHub AI Trending Digest ${dateStr}` : `📈 GitHub AI 热榜日报 ${dateStr}`;
  ...
  return makeSavedReport(
    lang === "en" ? "ai-trending-en" : "ai-trending",
    lang === "en" ? "GitHub AI Trending Digest" : "GitHub AI 热榜日报",
    fileName,
    trendingContent,
    lang,
  );
...
async function saveHnReport(
  ...
  try {
    ...
    return makeSavedReport(
      lang === "en" ? "ai-hn-en" : "ai-hn",
      lang === "en" ? "Hacker News AI Community Digest" : "Hacker News AI 社区动态日报",
      fileName,
      hnContent,
      lang,
    );
...
async function saveIntegratedDailyReport(
  ...
  if (reports.length === 0) {
    console.log(`  [daily/${lang}] No source reports available, skipping integrated daily.`);
    return;
  }

  console.log(`  [daily/${lang}] Calling LLM for integrated daily report...`);
  try {
    const dailySummary = await callLlm(buildIntegratedDailyPrompt(reports, dateStr, lang), 8192);
    const fileName = lang === "en" ? "ai-daily-en.md" : "ai-daily.md";
    const sourceLines = reports.map((report) => `- ${report.title}: \`${report.filename}\``).join("\n");
    const header =
      lang === "en"
        ? `# AI Ecosystem Daily Brief ${dateStr}\n\n` +
          `> Sources: ${reports.length} generated report(s) | Generated: ${utcStr} UTC\n\n` +
          `Source reports:\n${sourceLines}\n\n` +
          `---\n\n`
        : `# AI 生态整合日报 ${dateStr}\n\n` +
          `> 数据来源: ${reports.length} 份已生成报告 | 生成时间: ${utcStr} UTC\n\n` +
          `来源报告:\n${sourceLines}\n\n` +
          `---\n\n`;
    const dailyContent = header + dailySummary + footer;

    console.log(`  Saved ${saveFile(dailyContent, dateStr, fileName)}`);

    if (digestRepo) {
      const dailyTitle =
        lang === "en" ? `🗞️ AI Ecosystem Daily Brief ${dateStr}` : `🗞️ AI 生态整合日报 ${dateStr}`;
      const dailyLabel = lang === "en" ? "daily-en" : "daily";
      const dailyUrl = await createGitHubIssue(dailyTitle, dailyContent, dailyLabel);
      console.log(`  Created integrated daily issue (${lang}): ${dailyUrl}`);
    }
  } catch (err) {
    console.error(`  [daily/${lang}] Integrated daily generation failed: ${err}`);
  }
}
...
async function main(): Promise<void> {
  ...
  const zhDailyReports: DailySourceReport[] = [];
  const enDailyReports: DailySourceReport[] = [];
  ...
  if (genZh && zhSummaries) {
    ...
    zhDailyReports.push(
      makeSavedReport("ai-cli", "AI CLI 工具社区动态日报", "ai-cli.md", digestContent, "zh"),
      makeSavedReport("ai-skills", "Skills 生态热点", "ai-skills.md", skillsContent, "zh"),
      makeSavedReport("ai-agents", "AI Agents 生态日报", "ai-agents.md", openclawContent, "zh"),
    );
    if (digestRepo) {
  ...
  if (genEn && enSummaries) {
    ...
    enDailyReports.push(
      makeSavedReport("ai-cli-en", "AI CLI Tools Digest", "ai-cli-en.md", enDigestContent, "en"),
      makeSavedReport(
        "ai-skills-en",
  ...
  const [zhContentReports, enContentReports] = await Promise.all([
    genZh
      ? Promise.all(
  ...
  zhDailyReports.push(...zhContentReports.filter((report): report is SavedReport => report !== null));
  enDailyReports.push(...enContentReports.filter((report): report is SavedReport => report !== null));
  ...
  const [zhTrendingReport, enTrendingReport, zhHnReport, enHnReport] = await Promise.all([
    genZh && zhSummaries
      ? saveTrendingReport(
          trendingData,
  ...
  if (zhTrendingReport) zhDailyReports.push(zhTrendingReport);
  if (enTrendingReport) enDailyReports.push(enTrendingReport);
  if (zhHnReport) zhDailyReports.push(zhHnReport);
  if (enHnReport) enDailyReports.push(enHnReport);

  await Promise.all([
    genZh
      ? saveIntegratedDailyReport(zhDailyReports, utcStr, dateStr, digestRepo, footer, "zh")
      : Promise.resolve(),
    genEn
      ? saveIntegratedDailyReport(enDailyReports, utcStr, dateStr, digestRepo, enFooter, "en")
      : Promise.resolve(),
  ]);
