...
- 配置加载器现已同时支持旧版 schema 与新的 V2 `tracks[]` schema
...
- **旧版 schema**：`cli_repos`、`skills_repo`、`openclaw`、`openclaw_peers`（当前最稳定）
- **V2 `tracks[]` schema**：配置加载层已支持，可用于分阶段迁移；参考 [`config.tracks.example.yml`](./config.tracks.example.yml)
...
```
  ...
  如果你想开始迁移到 V2 配置模型，又不想立刻替换当前配置，建议先把 [`config.tracks.example.yml`](./config.tracks.example.yml) 复制到分支里单独演练。
